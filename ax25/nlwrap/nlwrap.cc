...
static void usage(const char* av0, int err)
{
    printf("Usage: %s: [-h] [-e <stderr file>] [-E <stderr file>] [command]\n", av0);
    exit(err);
}

static int open_stderr(const char* fn)
{
    if (fn == nullptr) {
        return -1;
    }
    ...
    if (ret == -1) {
        throw std::runtime_error(std::string("failed to open stderr file ") + fn + ": " +
                                 strerror(errno));
    }
    return ret;
}

static int mainloop(int pid, int fakeread, int fakewrite)
{
    ...
    for (;;) {
        ...
        if (!child_dead) {
            ...
            if (p2 != 0) {
                if (WIFEXITED(status)) {
                    fprintf(
                        stderr, "process exited with status %d\n", WEXITSTATUS(status));
                } else if (WIFSIGNALED(status)) {
                    fprintf(stderr, "process killed by signal %d\n", WTERMSIG(status));
                } else {
                    fprintf(stderr,
                            "huh, waitpid returned %d but not WIFEXITED? Tracing?\n",
                            p2);
                    continue;
                }
                child_dead = true;
...
static int wrapped_main(int argc, char** argv)
{
    ...
    if (pipe(fake0)) {
        fprintf(
            stderr, "%s: failed to create first pipe: %s\n", argv[0], strerror(errno));
        return EXIT_FAILURE;
    }
    if (pipe(fake1)) {
        fprintf(
            stderr, "%s: failed to create second pipe: %s\n", argv[0], strerror(errno));
        return EXIT_FAILURE;
    }
    ...
    case 0:
        ...
        if (child_stderr_file != -1) {
            if (-1 == dup2(child_stderr_file, STDERR_FILENO)) {
                throw std::runtime_error(std::string("dup2 into stderr: ") +
                                         strerror(errno));
            }
        }
    ...
    try {
        ...
        if (-1 == kill(pid, SIGTERM)) {
            fprintf(stderr,
                    "%s: Failed to kill child process during exception: %s\n",
                    argv[0],
                    strerror(errno));
        }
        throw;
